  ...
  return (
    ...
    >
      <View className="flex-row justify-between items-start mb-1">
        <Text className="text-muted font-medium">{mealType}</Text>
        <TouchableOpacity onPress={onEdit} className="flex-row items-center gap-1">
          <Edit width={14} height={14}/>
          <Text className="text-heading font-medium text-sm">Edit</Text>
        </TouchableOpacity>
      </View>
      ...
      {expanded && items.length > 0 && (
        <View className="border-t border-stroke pt-3 mb-3">
          {items.map((item) => (
            <MealItem key={item.id} name={item.name} icon={item.icon} amount={item.amount} />
          ))}
        </View>
      )}
      <View className="border-t border-stroke pt-4">
